...
2. **配方录入弹窗**：点击工具栏「+ 添加配方」，填写「输入物品数组 + 加工动作 + 输出产物」，提交后**自动生成节点与连线**，无需手动拖拽；输入 / 输出均可选择画布已有物品节点（复用而非新建重复节点）；选择已有加工节点时自动**复制**到新配方链（非链接原节点）；输入物品支持设置单位；附加操作下拉**动态同步**画布已有加工节点的 extra。「选择已有加工节点」下拉**按名称去重**（同名只展示最近创建的一个作为代表），选中即**直接复用该代表节点的图标**（无图标则留空），每个选项带**删除按钮**可移除该加工节点（画布节点 + 连线一并清除，正在编辑的目标节点不可删）。
...
6. **核心交互（配方链高亮）**：点击任意节点（物品 / 加工），双向遍历整张 DAG，找到该节点所处的**完整配方链**（向上追溯到最顶层原始原料、向下延伸到最终产物），将这条链上的所有节点与连线**高亮**、其余**置灰**，并自动将节点居中放大聚焦；点击画布空白区域取消高亮。
...
│   ├── useRecipeHighlight.ts    # 配方链高亮（上下游遍历，高亮完整链）
...
└── components/
    ├── FormPanel.vue            # 配方录入表单（弹窗内）：输入/输出可选已有节点、加工节点下拉去重+删除+图标复用、单位、分组、从分组复制属性
