...
3. **画布基础能力**：拖拽节点、平移缩放（Controls / MiniMap + 缩放指示）、框选、复制 / 粘贴、删除节点与连线（Delete 键或右键菜单）；**`Ctrl/⌘` + 左键点击节点立即复制**（副本生成新 ID、完整保留全部属性并偏移显示）；**`Ctrl/⌘` + 左键拖动节点则直接拖出副本**（原件始终钉在原位，副本跟随鼠标，松手落在终点）。
...
12. **快捷键**：`Ctrl + 左键点击节点` 立即复制、`Ctrl + 左键拖动节点` 复制并拖出副本、`Ctrl+A` 全选、`Ctrl+C / V` 复制粘贴、`Delete` 删除、`Esc` 取消选中 / 关闭弹窗（工具栏「⌨ 快捷键说明」可查看完整列表）。
